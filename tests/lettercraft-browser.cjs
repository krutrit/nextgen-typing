...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#lc-hearts').count(),0);
    assert.deepEqual(await page.evaluate(()=>[...new Set(Lettercraft.game.entities.filter(e=>e.kind==='animal').map(e=>e.species))].sort()),['chicken','cow','goat','pig','sheep']);
    ...
    assert.equal(await page.evaluate(()=>Lettercraft.game.collected),0);
    const holdState=await page.evaluate(()=>({collecting:Lettercraft.game.collecting,collected:Lettercraft.game.collected,keys:[...Lettercraft.game.keys],control:Lettercraft.hasControl(),hidden:document.hidden,background:Lettercraft.background}));
    assert.ok(holdState.collecting?.elapsed>2.5,JSON.stringify(holdState));
    ...
    const animalX=await page.evaluate(()=>{const g=Lettercraft.game;g.entities=[{id:950,kind:'animal',species:'pig',char:'a',x:g.player.x+2.5,y:g.player.y,z:g.player.z,angle:0,hp:3,maxHp:3,stun:0}];return g.entities[0].x;});
    await page.waitForFunction(x=>Lettercraft.game.entities[0].x>x+.4,animalX);
    assert.equal(await page.evaluate(()=>Lettercraft.game.entities[0].state),'flee');
    assert.deepEqual(await page.evaluate(()=>Lettercraft.game.enemies),[]);
    ...
    await page.evaluate(async()=>{state.lang='TH';loadLesson(0);await startMiniGame();});
    await page.locator('#lc-start-button').click();await locked();
    ...
    await page.evaluate(async()=>{const canvas=document.getElementById('lc-canvas'),original=canvas.getContext.bind(canvas);canvas.getContext=(type,...args)=>type==='webgl'?null:original(type,...args);await startMiniGame();});
    ...
    console.log('PASS: third-person orbit/Esc, crosshair mining/occlusion, collision, WASD/arrows, tools, hold while looking, five friendly species/fleeing/no hearts, timeout/retry/win, Thai marks, responsive layout, cleanup and no remote writes.');
